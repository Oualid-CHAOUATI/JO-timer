...
import { NumbersWrapper } from "./numbers-wrapper/Numbers-wrapper.components";

export function Timer({ interval = 6000 }) {
    ...
    <>
      <div className="timer-wrapper">
        <div className="timer">
          ...
          <div className={`events-wrapper ${showPara && "animate"}`}>
            <p>
              <span>{"jusqu'aux jeux olympiques"}</span>
              <span>{"de paris"}</span>
            </p>
            <p>
              <span>{"jusqu'aux jeux paralympiques "}</span>
              <span>{"de paris"}</span>
            </p>
          </div>

          <NumbersWrapper showPara={showPara} />
        </div>
      </div>
    </>
  );
